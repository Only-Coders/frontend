<template>
  <div>
    ...
    <v-row justify="space-around" align="center" no-gutters>
      ...
      <v-col cols="4">
        <h1 class="mb-4">{{ $i18n.t("Login.loginTitle") }}</h1>
        <v-form>
          <v-text-field v-model="email" :rules="emailRules" label="Email" rounded filled style="max-width: 450px">
          </v-text-field>
          <v-text-field v-model="password" :rules="emailRules" label="Password" rounded filled style="max-width: 450px">
          </v-text-field>
          <v-btn>{{ $i18n.t("Login.loginButton") }}</v-btn>
          <v-btn>{{ $i18n.t("Login.signInButton") }}</v-btn>
        </v-form>
      </v-col>
