<template>
  <div v-if="validUrl">
    ...
    <slot v-else :title="preview.title" :img="preview.img" :description="preview.description" :domain="preview.domain">
...
export default {
  ...
  data() {
    return {
      loading: false,
      preview: null,
      api: "https://api-dev.onlycoders.tech/",
      validUrl: false
    };
  },
  computed: {
    style() {
      const { width, maxWidth, marginRight, marginLeft, marginTop, marginBottom } = this;
      return {
        width,
        maxWidth,
        marginTop,
